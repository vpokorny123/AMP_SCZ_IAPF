...
#let's first look at reliability
cor.test(merged_df$paf_eyes_closed_bl,merged_df$paf_eyes_closed_fu)
cor.test(merged_df$paf_eyes_open_bl,merged_df$paf_eyes_open_fu)
...
pub_ready_stats(lmer(paf ~ phenotype + eyes_condition + Age + sex +
                       (1|src_subject_id),
                     data= merged_df_long))
